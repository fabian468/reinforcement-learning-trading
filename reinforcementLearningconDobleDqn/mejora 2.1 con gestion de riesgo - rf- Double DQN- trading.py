...
class AI_Trader():
    def __init__(self, state_size, action_space=3, model_name="AITrader",
                 max_open_positions=3, stop_loss_percent=0.01, take_profit_percent=0.02,
                 risk_per_trade_percent=0.01):
        self.state_size = state_size
        self.action_space = action_space
        self.memory = deque(maxlen=500)
        self.inventory = [] # Lista de diccionarios con info de compra (price, sl, tp, size)
        ...
        self.initial_equity = 1.0 # Se actualizará al inicio de cada episodio
    ...
    def trade(self, state):
        ...
        # Apply position limit here to influence the action
        if action == 1 and self.open_positions >= self.max_open_positions:
            action = 0  # Override 'buy' action if limit reached
    ...
    def save_model(self, name):
        ...
        with open(f"{name}_params.txt", "w") as f:
            ...
            f.write(f"max_open_positions:{self.max_open_positions}\n")
            f.write(f"stop_loss_percent:{self.stop_loss_percent}\n")
            f.write(f"take_profit_percent:{self.take_profit_percent}\n")
            f.write(f"risk_per_trade_percent:{self.risk_per_trade_percent}\n")
                    ...
                    for line in f:
                        if ":" in line:
                            ...
                            if key == "epsilon":
                                ...
                                self.max_open_positions = int(value)
                            elif key == "stop_loss_percent":
                                self.stop_loss_percent = float(value)
                            elif key == "take_profit_percent":
                                self.take_profit_percent = float(value)
                            elif key == "risk_per_trade_percent":
                                self.risk_per_trade_percent = float(value)
...
def plot_trading_session(data, buy_points, sell_points, sl_tp_lines, symbol, timeframe, save_path='resultados_cv'):
    ...
    for line in sl_tp_lines:
        plt.hlines(line['level'], line['start'], line['end'], colors=line['color'], linestyles='dashed', label=line['label'])
...
def main():
    ...
    trader = AI_Trader(state_size, max_open_positions=max_open_positions,
    ...
    for fold in range(n_folds):
        ...
        trader.initial_equity = 1.0 # Reset equity for each fold
        ...
        for episode in range(1, episodes + 1):
            ...
            winning_profits = []
            losing_profits = []
            peak_equity = trader.initial_equity
            current_equity = trader.initial_equity
            ...
            for t in range(data_samples):
                ...
                reward = 0
                profit = 0  # Inicializa 'profit' aquí
                current_price = fold_data['close'].iloc[t].item()
                timestamp = fold_data.index[t]

                if action == 1:  # Comprar
                    if trader.open_positions < trader.max_open_positions:
                        stop_loss_level = current_price * (1 - trader.stop_loss_percent)
                        take_profit_level = current_price * (1 + trader.take_profit_percent)
                        position_size = current_equity * trader.risk_per_trade_percent / abs(current_price - stop_loss_level) if abs(current_price - stop_loss_level) > 0 else 0
                        if position_size > 0:
                            buy_price = current_price + trader.spread / 2
                            trader.inventory.append({'price': buy_price, 'sl': stop_loss_level, 'tp': take_profit_level, 'size': position_size, 'timestamp': timestamp})
                            trader.open_positions += 1
                            trades_count += 1
                            current_equity -= buy_price * position_size * (1 + trader.commission_per_trade)
                            if episode == episodes and fold == n_folds - 1:
                                buy_points.append((timestamp, buy_price))
                                sl_tp_lines_episode.append({'level': stop_loss_level, 'start': timestamp, 'end': fold_data.index[-1], 'color': 'red', 'label': 'SL'})
                                sl_tp_lines_episode.append({'level': take_profit_level, 'start': timestamp, 'end': fold_data.index[-1], 'color': 'green', 'label': 'TP'})
                    else:
                        print(f"Límite de posiciones ({trader.max_open_positions}) alcanzado en {timestamp}. No se puede comprar.")

                elif action == 2 and trader.inventory:  # Vender (cierre manual)
                    best_trade_idx = 0
                    if len(trader.inventory) > 1:
                        # Simple strategy: close the oldest position
                        pass
                    position_to_close = trader.inventory.pop(best_trade_idx)
                    sell_price = current_price - trader.spread / 2
                    profit = (sell_price - position_to_close['price']) * position_to_close['size']
                    reward = profit
                    total_profit += profit
                    trades_count += 1
                    current_equity += sell_price * position_to_close['size'] * (1 - trader.commission_per_trade)
                ...
                inventory_copy = trader.inventory[:]
                for i, position in enumerate(inventory_copy):
                    close_position = False
                    profit_sl_tp = 0
                    sell_price_sl_tp = 0
                    if current_price <= position['sl']:
                        sell_price_sl_tp = position['sl']
                        profit_sl_tp = (sell_price_sl_tp - position['price']) * position['size']
                        print(f"Stop-loss en {symbol} a {sell_price_sl_tp:.5f} en compra a {position['price']:.5f} en {timestamp}")
                        close_position = True
                    elif current_price >= position['tp']:
                        sell_price_sl_tp = position['tp']
                        profit_sl_tp = (sell_price_sl_tp - position['price']) * position['size']
                        print(f"Take-profit en {symbol} a {sell_price_sl_tp:.5f} en compra a {position['price']:.5f} en {timestamp}")
                        close_position = True

                    if close_position:
                        total_profit += profit_sl_tp
                        trades_count += 1
                        current_equity += sell_price_sl_tp * position['size'] * (1 - trader.commission_per_trade)
                        episode_returns.append(profit_sl_tp)
                        trader.open_positions -= 1
                        if profit_sl_tp > 0:
                            wins += 1
                            winning_profits.append(profit_sl_tp)
                        else:
                            losses += 1
                            losing_profits.append(profit_sl_tp)
                        if episode == episodes and fold == n_folds - 1:
                            sell_points.append((timestamp, sell_price_sl_tp))
                        # Remove the closed position
                        trader.inventory = [item for idx, item in enumerate(trader.inventory) if idx != i]
            ...
            print(f"Fin Episodio {episode}: Beneficio={price_format(total_profit)}, Trades={trades_count}, Sharpe={sharpe:.2f}, Drawdown={max_drawdown:.2%}, Accuracy={accuracy:.2%}, Open Positions={trader.open_positions}")
            ...
            if episode == episodes and fold == n_folds - 1 and (buy_points or sell_points):
                plot_trading_session(fold_data, buy_points, sell_points, sl_tp_lines_episode, symbol, intervalo, save_path=resultados_dir)
    ...
    if len(test_data) > window_size:
        ...
        for t in range(test_samples):
            ...
            if test_action == 1:
                if test_open_positions < max_open_positions:
                    stop_loss_level_test = current_price * (1 - stop_loss_percent)
                    take_profit_level_test = current_price * (1 + take_profit_percent)
                    position_size_test = test_current_equity * risk_per_trade_percent / abs(current_price - stop_loss_level_test) if abs(current_price - stop_loss_level_test) > 0 else 0
                    if position_size_test > 0:
                        buy_price_test = current_price + trader.spread / 2
                        test_inventory.append({'price': buy_price_test, 'sl': stop_loss_level_test, 'tp': take_profit_level_test, 'size': position_size_test, 'timestamp': timestamp})
                        test_open_positions += 1
                        test_trades += 1
                        test_current_equity -= buy_price_test * position_size_test * (1 + trader.commission_per_trade)
                        test_buy_points.append((timestamp, buy_price_test))
                        test_sl_tp_lines.append({'level': stop_loss_level_test, 'start': timestamp, 'end': test_data.index[-1], 'color': 'red', 'label': 'SL'})
                        test_sl_tp_lines.append({'level': take_profit_level_test, 'start': timestamp, 'end': test_data.index[-1], 'color': 'green', 'label': 'TP'})
                else:
                    print(f"Límite de posiciones ({max_open_positions}) alcanzado en prueba en {timestamp}. No se puede comprar.")

            elif test_action == 2 and test_inventory:
                position_to_close_test = test_inventory.pop(0)
                sell_price_test = current_price - trader.spread / 2
                profit_test = (sell_price_test - position_to_close_test['price']) * position_to_close_test['size']
                test_profit += profit_test
                test_trades += 1
                test_current_equity += sell_price_test * position_to_close_test['size'] * (1 - trader.commission_per_trade)
            ...
            # Check SL/TP for test set
            inventory_copy_test = test_inventory[:]
            for i, position in enumerate(inventory_copy_test):
                close_position_test = False
                profit_sl_tp_test = 0
                sell_price_sl_tp_test = 0
                if current_price <= position['sl']:
                    sell_price_sl_tp_test = position['sl']
                    profit_sl_tp_test = (sell_price_sl_tp_test - position['price']) * position['size']
                    print(f"Stop-loss en prueba en {symbol} a {sell_price_sl_tp_test:.5f} en compra a {position['price']:.5f} en {timestamp}")
                    close_position_test = True
                elif current_price >= position['tp']:
                    sell_price_sl_tp_test = position['tp']
                    profit_sl_tp_test = (sell_price_sl_tp_test - position['price']) * position['size']
                    print(f"Take-profit en prueba en {symbol} a {sell_price_sl_tp_test:.5f} en compra a {position['price']:.5f} en {timestamp}")
                    close_position_test = True

                if close_position_test:
                    test_profit += profit_sl_tp_test
                    test_trades += 1
                    test_current_equity += sell_price_sl_tp_test * position['size'] * (1 - trader.commission_per_trade)
                    test_returns.append(profit_sl_tp_test)
                    test_open_positions -= 1
                    if profit_sl_tp_test > 0:
                        wins_test += 1
                        winning_profits_test.append(profit_sl_tp_test)
                    else:
                        losses_test += 1
                        losing_profits_test.append(profit_sl_tp_test)
                    test_sell_points.append((timestamp, sell_price_sl_tp_test))
                    test_inventory = [item for idx, item in enumerate(test_inventory) if idx != i]
        ...
        print(f"Resultados en Prueba: Beneficio={price_format(test_profit)}, Trades={test_trades}, Sharpe={test_sharpe:.2f}, Drawdown={test_max_drawdown:.2%}, Accuracy={test_accuracy:.2%}, Open Positions={test_open_positions}")
        plot_trading_session(test_data, test_buy_points, test_sell_points, test_sl_tp_lines, symbol, intervalo, save_path=resultados_dir)
