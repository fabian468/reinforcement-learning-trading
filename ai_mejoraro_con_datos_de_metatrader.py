...
def plot_trading_session(data, buy_points, sell_points, symbol, timeframe):
    ...
    plt.plot(data.index, data.values, label=f'{symbol} - {timeframe}')
...
def main():
    ...
    cargar_modelo = True  # Cambiar a True para cargar un modelo existente
    ...
    for episode in range(1, episodes + 1):
        ...
        if episode % 25 == 0:
            trader.save_model(f"ai_trader_{episode}_{intervalo}")
